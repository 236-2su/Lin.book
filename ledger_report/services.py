...
def generate_report_advice_with_llm(report_data: dict) -> str:
    ...
    system_prompt = "당신은 전문적인 동아리 재무 분석가입니다. 제공된 연간 재무 보고서 데이터를 바탕으로, 동아리 운영진이 다음 해 재무 계획을 세우는 데 도움이 될 만한 상세하고 구체적인 조언을 한국어로 작성해 주세요."
...
[요청 사항]
위 데이터를 심층적으로 분석하여 다음 내용을 포함하는 재무 분석 보고서 및 조언을 작성해 주세요.
...
결과는 마크다운 형식을 사용하여 가독성 좋게 작성해 주세요.
"""
