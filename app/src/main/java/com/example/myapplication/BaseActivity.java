...
public abstract class BaseActivity extends AppCompatActivity {
    ...
    private void setupHeader() {
        ...
        // 로고 클릭 시 클럽 목록 화면으로 이동
        if (logoImage != null) {
            logoImage.setOnClickListener(v -> {
                if (this instanceof MainActivity) {
                    // 메인에서라면 클럽 리스트 프래그먼트로 교체
                    ((MainActivity) this).replaceFragment(new ClubListFragment());
                } else {
            ...
            btnPublicAccount.setOnClickListener(v -> {
                ...
                if (this instanceof MainActivity) {
                    ...
                    if (currentClubId > 0) {
                        ...
                        ((MainActivity) this).fetchLedgerIdAndShowFragment(currentClubId);
                    } else {
